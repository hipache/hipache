    ...
    Config.prototype.prepareHttps = function (https) {
        ...
        https.bind = https.bind.map(function (item) {
            ...
            [
                'port',
                'key', 'cert', 'ca',
                'keyContent', 'certContent', 'caContent', 'sni',
            ...
            if ((!item.cert || !item.key) && (!item.certContent || !item.keyContent)) {
                return this.emit('error', new ConfigError(
            ...
            if (item.sni) {
                for (var key in item.sni) {
                    if (item.sni.hasOwnProperty(key)) {
                        var sniItem = item.sni[key];
                        sniItem.ca = sniItem.caContent || sniItem.ca.map(function (ca) {
                            return fs.readFileSync(ca, 'utf8');
                        });
                        sniItem.cert = sniItem.certContent || fs.readFileSync(sniItem.cert, 'utf8');
                        sniItem.key = sniItem.keyContent || fs.readFileSync(sniItem.key, 'utf8');
                    }
                }
            }
            ...
            try {
                item.ca = item.caContent || item.ca.map(function (ca) {
                    return fs.readFileSync(ca, 'utf8');
                });
                item.cert = item.certContent || fs.readFileSync(item.cert, 'utf8');
                item.key = item.keyContent || fs.readFileSync(item.key, 'utf8');
            } catch (e) {
