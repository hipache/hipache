...
Cache.prototype.getBackendFromHostHeader = function (host, callback) {
    ...
    readFromCache(host, function gotCache(rows) {
        var backends = rows[0];
        ...
        // application name always goes first in backends list in Redis
        var appName = backends.shift();

        // [appName, 'paused']
        if (backends[0] === 'paused') {
            if (resumeDone === false) {
                resumeDone = true;
                ...
                api.resume({
                    ...
                    readFromCache(host, gotCache);
                });
            } else if (resumeDone === true && lookups++ < self.config.api.maxLookups) {
                setTimeout(function delayedRedisLookup() {
                    readFromCache(host, gotCache);
                }, self.config.api.lookupInterval);
            } else {
                return callback('frontend not found', 400);
            }
            return;
        ...
        // [appName, backend1, backend2, ..., backendN]
        var virtualHost = appName;
        ...
        // TODO make HTTP API call here and call
        ...
        self.debug('Proxying: ' + host + ' -> ' + backend.hostname + ':' + backend.port);
