...
Cache.prototype.getBackendFromHostHeader = function (host, callback) {
    ...
    var processHost = function (rows) {
        ...
        if (!indexes.length) {
            // if the next backend is a wildcard, try it before throw an 502 error
            var next = rows.shift();
            if (next[0] === '*' && virtualHost !== '*') {
                return readFromCache('*.rocket.chat', processHost);
            }
    ...
    readFromCache(host, processHost);
};
