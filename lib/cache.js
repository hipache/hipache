...
Cache.prototype.getBackendFromRequest = function (req, callback) {
    ...
    readFromCache(host, function (rows) {
        ...
        console.log("Dead backends: " + deads);
              ...
              if (bcd.type === 'req.headers.host') {
                ...
                bcd.url.forEach(function(b) {
                  if (deads.indexOf(i) === -1) {
                    validBackends.push({url: b, idx: i});
                  } else {
                    console.log("WARNING: skipping dead backend: " + i);
                  }
                  i++;
                });
              }
        ...
        var totalBackendsLength = i;

        // console.log(validBackends);
        ...
        backend.len = totalBackendsLength;
        if (!backend.hostname) {
