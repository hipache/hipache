...
function parseSubdomain(host) {
  ...
  if (!(host.match(/^[^-]{1,}-\d{1,5}./))) {
    // If this is true, then the port was not specified in the subdomain already
    var split =  host.indexOf('.');
    var subdomain = host.slice(0, split);
    host = subdomain + '-' + port + host.slice(split);
  }
  return host;
}
...
module.exports.parseSubdomain = parseSubdomain;
