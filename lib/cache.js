...
function Cache(config, handlers) {
    ...
    // default greedy wildcard to true if it is not set
    this.config.greedyWildcard = (this.config.greedyWildcard === false) ? false : true;
...
Cache.prototype.getDomainName = function (hostname) {
    ...
    if (this.config.server.greedyWildcard) {
