    ...
    var httpRequestHandler = function (req, res) {
        ...
        var proxyRequest = function () {
            ...
            this.cache.getBackendFromHostHeader(req.headers.host, function (err, code, backend) {
                ...
                var paths = util._extend([], config.stack);

                (function resolve(paths, stack, run) {
                    return paths.length
                        ? require(paths.pop())(this, req, stack, function() { resolve(paths, stack, run); })
                        : run(stack);
                }.bind(this))(paths, new Array(), function(stack) {
                    stack.push(function(req, res) {
                        proxy.proxyRequest(req, res, buffer);
                    });

                    httpProxy.stack(stack, proxy)(req, res);
                });
            }.bind(this));
        }.bind(this);
        if (config.retryOnError) {
