...
Worker.prototype.runServer = function (config) {
    ...
    var httpRequestHandler = function (req, res, isHttps) {
        if (isHttps !== true && config.http && config.http.forceHTTPS === true) {
    ...
    var httpsRequestHandler = function (req, res) {
        return httpRequestHandler(req, res, true)
    }.bind(this);
    ...
    config.https.bind.forEach(function (options) {
        ...
        if (options.sni) {
            var sni = options.sni;
            delete options.sni;
            options.SNICallback = function (domain, cb) {
                var credentials = sni[domain];
                if (!credentials) {
                    credentials = sni[domain.replace(/^[^.]+\./, '')]
                }
                if (!credentials) {
                    return cb(new Error);
                }
                ...
                if (!credentials.context) {
                    credentials.context = tls.createSecureContext({
                        key: credentials.key,
        ...
        var httpsServer = https.createServer(options, httpsRequestHandler);
