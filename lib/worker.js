...
Worker.prototype.runServer = function (config) {
    ...
    var reStreamer = function (){
      return function (req, res, next) { //restreame
        ...
        process.nextTick(function () {
          if(req.body) {
            req.emit('data', req.body); // Need to URL Encode this??
          }
          req.emit('end');
        });
        next();
    ...
    var httpRequestHandler = connect()
      .use(bodyParser.urlencoded({ extended: false })) //form post
      .use(reStreamer())
        ...
        (function () {
            ...
            res.writeHead = function (statusCode) {
                ...
                var markDeadBackend = function () {
                    // console.log("MARKING DEAD BACKEND: " + req.meta.backendId);
    ...
    var wsRequestHandler = function (req, socket, head) {
        this.cache.getBackendFromRequest(req, function (err, code, backend) {
            if (err) {
                // XXX: log here for websocket errors.
