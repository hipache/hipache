...
function Worker(config) {
    ...
    var debug   = debug = ll.debug(config.server.debug)
...
Worker.prototype.runServer = function (config) {
    ...
    var proxyErrorHandler = function (err, req, res) {
        if (err.code === 'ECONNREFUSED' ||
            ...
            logger.logit(req.headers.host + ': backend #' + backendId + ' is dead (' + JSON.stringify(err) + ') while handling request for ' + req.url);
        } else {
