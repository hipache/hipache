...
function Worker(config) {
    ...
    var logger  = new Logger(config.server.accessLog)
    var log     = log   = logger.logMessage
    var debug   = debug = logger.debug(config.server.debug)
...
Worker.prototype.runServer = function (config) {
    ...
    var proxyErrorHandler = function (err, req, res) {
        if (err.code === 'ECONNREFUSED' ||
            ...
            logger.logMessage(req.headers.host + ': backend #' + backendId + ' is dead (' + JSON.stringify(err) + ') while handling request for ' + req.url);
        } else {
