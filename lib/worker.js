...
Worker.prototype.runServer = function (config) {
    ...
    var addMiddlewaresFromConfig = function(app, config) {
      for (var middlewareName in config.server.middleware) {
        var middlewareConfig = config.server.middleware[middlewareName];
        if (middlewareConfig.enabled) {
          log('Middleware ' + middlewareName + " is enabled!");
          var module = require(middlewareConfig.module.name);
          var method = middlewareConfig.module.method;
          var args = middlewareConfig.module.arguments;
          var middlewareFunc = module[method].apply(this, args);
    ...
    var app = connect();
    addMiddlewaresFromConfig(app, config);
